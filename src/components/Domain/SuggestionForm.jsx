...
export default function SuggestionForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="form-card fade-in">
      ...
      <Input
        id="reflection"
        label="Pergunta para Reflexão (Opcional)"
        value={formData.reflection}
        onChange={(e) => handleChange('reflection', e.target.value)}
        placeholder="Ex: Como o amor de Deus impacta a forma como reajo aos problemas?"
        textarea
        rows={2}
      />
      ...
      <Input
        id="prayer"
        label="Oração Curta (Opcional)"
        value={formData.prayer}
        onChange={(e) => handleChange('prayer', e.target.value)}
        placeholder="Ex: Senhor, ensina-me a andar sempre sob a direção do Teu amor..."
        textarea
        rows={2}
      />
